...
class SbahnObject:
    def __init__(self, requestjson, ypos):
        ...
        self.when_int = wheninminutes(requestjson)
    ...
    def departure_list(self):
        ...
        for i,displayedobject in enumerate(self.dObs[1:]):
            if i<len(s_bahn_departures):
                ...
                inminutes = wheninminutes(s_bahn)
...
def setup(canvas, images, HUlogoimage):
    canvas.create_rectangle(580, 0, 1200, 800, fill='light blue', outline='light blue')
    canvas.create_image(700, 100, image=HUlogoimage)
...
for j, sa in enumerate(s):
    if j > 0:
        offset += stations[j-1].get_dep_list_length() + 1
